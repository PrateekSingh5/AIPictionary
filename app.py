# -*- coding: utf-8 -*-
"""
Created on Mon Nov 30 18:03:20 2020

@author: Prateek-Ubuntu
"""
...
#def df_to_image_array_xxd(arr, size, lw=6, time_color=True):
#	x = np.zeros((1, size, size, 1))
#for i, raw_strokes in enumerate(arr):
#	print(arr[0])
#	x[0, :, :, 0] = draw_cv22(arr[0], size=size, lw=lw, time_color=time_color)
...
def df_to_image_array_xxd(arr, size, lw=6, time_color=True):
    x = np.zeros((1, size, size, 1))
#    for i, raw_strokes in enumerate(arr):
    x[0, :, :, 0] = draw_cv2(arr, size=size, lw=lw, time_color=time_color)
    x = preprocess_input(x).astype(np.float32)
    return x
...
def predict(testing):
	x_test = df_to_image_array_xxd(testing, size)
	#print(x_test)
	model = keras.models.load_model('mymodel', custom_objects=dependencies)
...
#	print(cats)
	...
	Temp = top3cats['a'] + ' ' + top3cats['b'] + ' ' + top3cats['c']

	# make dict here.
	return Temp.tolist()
#	return top3cats.values.tolist()
...
def contact():
    ...
    return render_template('contact.html',catagory=name)
    
    
    
    
@app.route('/canvas-predict',methods=["POST"])
def canvasPredict():
	req_data = request.get_json()
	dataset = req_data['dataset']
#	print(dataset)

	return {"Predictions" : predict(dataset)}
